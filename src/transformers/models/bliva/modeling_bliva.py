...
from .configuration_bliva import BlivaConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "TODO-org/TODO-base-cased"
_CONFIG_FOR_DOC = "BlivaConfig"


BLIVA_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "TODO-org/TODO-base-cased",
...
class BlivaLearnedPositionalEmbedding(nn.Embedding):
    """
...
class BlivaDecoderLayer(nn.Module):
    def __init__(self, config: BlivaConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim)
        self.encoder_attn = BlivaAttention(
            self.embed_dim,
...
class BlivaClassificationHead(nn.Module):
...
class BlivaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (BlivaDecoder, BlivaEncoder)):
...
BLIVA_GENERATION_EXAMPLE = r"""
    Summarization example:

    ```python
    >>> from transformers import BlivaTokenizer, BlivaForConditionalGeneration

    >>> model = BlivaForConditionalGeneration.from_pretrained('TODO-org/TODO-base-cased')
    >>> tokenizer = BlivaTokenizer.from_pretrained('TODO-org/TODO-base-cased')
...
BLIVA_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            ...
            Indices can be obtained using [`~BlivaTokenizer`]. See
        ...
        decoder_attention_mask (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            If you want to change padding behavior, you should read [`modeling_bliva._prepare_decoder_attention_mask`] and
...
BLIVA_STANDALONE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
class BlivaEncoder(BlivaPreTrainedModel):
    """
    ...
    def __init__(self, config: BlivaConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = BlivaLearnedPositionalEmbedding(
            config.max_position_embeddings,
            embed_dim,
        )
        self.layers = nn.ModuleList([BlivaEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class BlivaDecoder(BlivaPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`BlivaDecoderLayer`]
    ...
    def __init__(self, config: BlivaConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BlivaDecoderLayer(config) for _ in range(config.decoder_layers)])
...
@add_start_docstrings(
    "The bare BLIVA Model outputting raw hidden-states without any specific head on top.",
...
class BlivaModel(BlivaPreTrainedModel):
    def __init__(self, config: BlivaConfig):
        ...
        self.encoder = BlivaEncoder(config, self.shared)
        self.decoder = BlivaDecoder(config, self.shared)
...
@add_start_docstrings(
    "The BLIVA Model with a language modeling head. Can be used for summarization.", BLIVA_START_DOCSTRING
)
class BlivaForConditionalGeneration(BlivaPreTrainedModel):
    ...
    @add_end_docstrings(BLIVA_GENERATION_EXAMPLE)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
...
@add_start_docstrings(
    """
    Bliva model with a sequence classification/head on top (a linear layer on top of the pooled output) e.g. for GLUE
    tasks.
    """,
    BLIVA_START_DOCSTRING,
)
class BlivaForSequenceClassification(BlivaPreTrainedModel):
    def __init__(self, config: BlivaConfig, **kwargs):
        super().__init__(config, **kwargs)
        self.model = BlivaModel(config)
        self.classification_head = BlivaClassificationHead(
            config.d_model,
            config.d_model,
            config.num_labels,
...
@add_start_docstrings(
    """
    BLIVA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
    layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    BLIVA_START_DOCSTRING,
)
class BlivaForQuestionAnswering(BlivaPreTrainedModel):
...
class BlivaDecoderWrapper(BlivaPreTrainedModel):
    """
    ...
    def __init__(self, config):
        super().__init__(config)
        self.decoder = BlivaDecoder(config)

    def forward(self, *args, **kwargs):
        return self.decoder(*args, **kwargs)


class BlivaForCausalLM(BlivaPreTrainedModel):
    def __init__(self, config):
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        super().__init__(config)
        self.model = BlivaDecoderWrapper(config)
    ...
    def forward(
        ...
        ```python
        >>> from transformers import BlivaTokenizer, BlivaForCausalLM

        >>> tokenizer = BlivaTokenizer.from_pretrained('facebook/bart-large')
        >>> model = BlivaForCausalLM.from_pretrained('facebook/bart-large', add_cross_attention=False)
